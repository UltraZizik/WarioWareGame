...
@onready var apple: TextureRect = $LivesContainer/Apple
@onready var apple_2: TextureRect = $LivesContainer/Apple2
@onready var apple_3: TextureRect = $LivesContainer/Apple3
@onready var apple_4: TextureRect = $LivesContainer/Apple4
@onready var apple_5: TextureRect = $LivesContainer/Apple5
@onready var timer: RichTextLabel = $Timer
@onready var level: RichTextLabel = $Level

var time
func _ready() -> void:
	await Timer(5.0)#Function
	
	if Global.minigames_done < 3: # if you havent completed 3 minigames yet 
		Global.minigames_done = Global.minigames_done +1
		get_tree().change_scene_to_file("res://files/scenes/minigame_" + str(Global.minigames_done) + ".tscn") # changes your scene by arranging this frankenstein path. 
